...
def sternum_envelope_line(sternum_df, arr_shape, center_line, width=100, hu_threshold=400, output_prefix=None):
    ...
    sternum_df = sternum_df[(sternum_df['y'] > y_limit_min) & (sternum_df['y'] < y_limit_max)]
    ...
    sternum_df_group_by_z[sternum_df_group_by_z['z'] < 4*len(sternum_df)//5]['y.min'] = \
        sternum_df_group_by_z[sternum_df_group_by_z['z'] < 4*len(sternum_df)//5]['y.min'].apply(lambda x: None if x < y_limit_min + 15 else x)
    sternum_df_group_by_z[sternum_df_group_by_z['z'] < 4*len(sternum_df)//5]['y.max'] = \
        sternum_df_group_by_z[sternum_df_group_by_z['z'] < 4*len(sternum_df)//5]['y.max'].apply(lambda x: None if x > y_limit_max - 15 else x)
    ...
    y_envelope_min = pd.Series(y_envelope_min).fillna(method='bfill').fillna(method='ffill').rolling(10, min_periods=5).mean()
    y_envelope_max = pd.Series(y_envelope_max).fillna(method='bfill').fillna(method='ffill').rolling(10, min_periods=5).mean()

    y_envelope_min.apply(lambda x: x-10)
    y_envelope_max.apply(lambda x: x+10)
    """
    plt.figure()
...
def sternum_cut(value_arr, sternum_envelope_line_left, sternum_envelope_line_right):
    ...
    z_index = []
    y_index = []
    for i in range(len(sternum_envelope_line_left)):
    ...
    for i in range(3*value_arr.shape[1]//5):
