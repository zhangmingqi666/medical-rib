...
class SternumRemove:
    ...
    def set_y_min_max_for_every_z(self, rolling_window=20, min_periods=10):
        ...
        if self.allow_envelope_expand:                         # expand the line width
    ...
    def sternum_remove_operation(self, value_arr=None):
        """
        remove sternum form CT source array
        :param value_arr:
        :return:
        """
        ...
        while x_min <= x_max:    # remove a slice during a remove time
    ...
    def cut_sternum_v2(self):
        ...
        z_count_list = y_line['z.count'].values
        z_count_backward_shift = z_count_list[-1:] + z_count_list[:-1]
        z_count_derivative = np.array(z_count_backward_shift) - np.array(z_count_list)
        plt.plot(y_line['y'], z_count_derivative)
        plt.show()
