...
def judge_collect_spine_judge_connected_rib(sparse_df=None, cluster_df=None, bone_prior=None):
    ...
    loc_sternum_connected_rib = False
    for e in cluster_df['c'].values:
        temp_sparse_df = sparse_df[sparse_df['c'] == e]
        # will add center line , @issac
        single_bone = Bone(bone_data=temp_sparse_df, arr_shape=bone_prior.get_prior_shape(), spine_width=100,
        ...
        if single_bone.is_spine():
            ...
            if single_bone.sternum_connected_rib():
                loc_sternum_connected_rib = True
        del single_bone
    # print(sternum_bone_df)

    return loc_spine_connected_rib, loc_sternum_connected_rib, remaining_bone_df, sternum_bone_df
...
def loop_opening_get_spine(binary_arr, hu_threshold=400, bone_prior=None, allow_debug=False):
    ...
    # calc bone prior
    # sternum_bone_df = pd.DataFrame({})
    last_glb_spine_connected_rib = True
    last_glb_sternum_connected_rib = True
    while True:
        ...
        with timer('_________collect spine and judge connected'):
            glb_spine_connected_rib, glb_sternum_connected_rib, remaining_bone_df, sternum_bone_df = judge_collect_spine_judge_connected_rib(sparse_df=sparse_df,
        ...
        # when glb_spine_connected_rib was triggered from True to False, we save them
        if glb_spine_connected_rib is False and last_glb_spine_connected_rib is True:
            glb_remaining_bone_df = remaining_bone_df
        last_glb_spine_connected_rib = glb_spine_connected_rib

        if glb_sternum_connected_rib is False and last_glb_sternum_connected_rib is True:
            glb_sternum_bone_df = sternum_bone_df
        last_glb_sternum_connected_rib = glb_sternum_connected_rib

        if glb_sternum_connected_rib:
            sparse_df = sparse_df.append(sternum_bone_df)
        if glb_spine_connected_rib:
            sparse_df = sparse_df.append(remaining_bone_df)

        del remaining_bone_df, sternum_bone_df
        if len(sparse_df) == 0:
            break

        with timer('_________sparse df to arr'):
            binary_arr = sparse_df_to_arr(arr_expected_shape=bone_prior.get_prior_shape(), sparse_df=sparse_df)
            binary_arr[binary_arr > 0] = 1

        # these variable will be be returned
        del sparse_df

        with timer('_________binary opening'):
            binary_arr = loop_morphology_binary_opening(binary_arr, use_cv=False)

    return glb_remaining_bone_df, glb_sternum_bone_df
...
def main_excute(value_arr, allow_debug=False, output_prefix='hello'):
    ...
    with timer('get spine remaining df'):
        ...
        plot_yzd(temp_df=remaining_bone_df, shape_arr=(shape[0], shape[2]), save=True,
                 save_path='{}remaining_bone.png'.format(output_prefix))
        if len(sternum_bone_df) == 0:
            print("肋骨被开运算没啦！！！")
        else:
            plot_yzd(temp_df=sternum_bone_df, shape_arr=(shape[0], shape[2]), save=True,
                     save_path='{}sternum_bone.png'.format(output_prefix))
    return
