...
def load_scan(path):
    ...
    """
    slices = [dicom.read_file(os.path.join(path, s), force=True) for s in os.listdir(path) if not s.startswith('._')]
...
def resample(image, scan, new_spacing=[1, 1, 1], keep_slicing=True):
    """
...
class RibDataFrame:
    ...
    def readDicom(self, path=None, keep_slicing=True):
        ...
        with timer("pix_resampled"):
            pix_resampled, new_spacing = resample(first_patient_pixels, first_patient, [1, 1, 1],
                                                  keep_slicing=keep_slicing)
        gc.collect()
        return pix_resampled, new_spacing
