...
def nii_read(nii_file_path=None, keep_slicing=True):
    """read box min,max from nii file"""
    img = nib.load(nii_file_path)
    header = img.header
    pixel_zoom = header.get_zooms()
    if keep_slicing:
        pixel_zoom = pixel_zoom / pixel_zoom[0]
...
def parse_args():
    ...
    parser.add_argument('--keep_slicing', dest='keep_slicing', action='store_true')
    parser.set_defaults(keep_slicing=False)
    args = parser.parse_args()
    print("keep_slicing is {}".format(args.keep_slicing))
...
if __name__ == '__main__':
    ...
    for folder in nii_folder_list:
        temp_df = location_read(folder_path=folder, keep_slicing=args.keep_slicing)
