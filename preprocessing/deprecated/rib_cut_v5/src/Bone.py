...
class Bone:
    ...
    def selective_initial(self):
        if self.detection_objective == 'rib':
            ...
            self.set_z_distance_on_x_centroid()
        else:
    ...
    """
    def set_z_distance_on_x_centroid(self):
        _, x_centroid, _ = self.get_basic_axis_feature(feature='centroid')
        z_centroid_df = self.bone_data[(self.bone_data['x'] > x_centroid - 3) & (self.bone_data['x'] < x_centroid + 3)]
        self.z_distance_on_x_centroid = z_centroid_df['z'].max() - z_centroid_df['z'].min()
        del z_centroid_df

    def get_z_distance_on_x_centroid(self):
        return self.z_distance_on_x_centroid
    ...
    def is_first_rib(self):
        if self.get_basic_axis_feature(feature='centroid')[0] < self.z_mid_line:
            return False

        if self.get_basic_axis_feature(feature='max')[0] > self.arr_shape[0] - 5:
            return False

        if self.get_basic_axis_feature(feature='length')[2] > self.y_mid_line:
            return False

        if self.get_z_distance_on_x_centroid() > self.rib_diameter * 2:
            return False
        ...
        if abs(y_centroid - self.y_mid_line) > 120:
                return False

        if self.get_distance_between_centroid_and_nearest_point() < 2:
            return False
    ...
    def is_mid_rib(self):
        ...
        if self.get_distance_between_centroid_and_nearest_point() < 12:
            return False

        if self.get_z_distance_on_x_centroid() > self.rib_diameter:
            return False

        z_centroid, x_centroid, _ = self.get_basic_axis_feature(feature='centroid')
        _, _, y_max = self.get_basic_axis_feature(feature='max')
        _, _, y_min = self.get_basic_axis_feature(feature='min')

        if self.bone_data[self.bone_data['x'] > x_centroid]['z'].mean() < z_centroid:
            return False
    ...
    def is_low_rib(self):
        ...
        _, _, y_max = self.get_basic_axis_feature(feature='max')
        _, _, y_min = self.get_basic_axis_feature(feature='min')
        if z_centroid > self.arr_shape[0]//2:
            return False
    ...
    def is_multi_rib(self):
        ...
        max_nonzero_internal, _ = self.detect_multi_ribs()
    ...
    def is_half_spine(self):
        # _, x_centroid, _ = self.get_basic_axis_feature(feature='centroid')
        _, x_local_centroid, _ = self.local_centroid_for_sternum
        if x_local_centroid < self.x_mid_line//2:
            return False
    ...
    def detect_multi_ribs(self):
        ...
        hollow_rate = len(z_distribution) / (z_distribution['z'].max() - z_distribution['z'].min())
        del z_distribution
        return max_nonzero_internal, hollow_rate
    ...
    def set_bone_type(self):
        ...
        if self.detection_objective == 'rib':
            ...
            if self.is_multi_rib():
                self.bone_type = BoneType.Rib_Bone
                self.rib_type = RibType.Multi_Rib
                # self.cut_other_rib()
                return
        else:
    ...
    def print_bone_info(self):
        ...
        print('max_internal and hollow rate:', self.detect_multi_ribs())
        print('shape:{}, basic axis info:{}'.format(self.arr_shape, self.get_basic_axis_feature(feature='centroid')))
